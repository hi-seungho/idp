...
$stw = '';
if ($member['mb_level'] == 3 && $bo_table == "reservation") {
    $stw .= " wr_1='{$member['mb_id']}'";
    if (isset($wr_id) && $wr_id) {
        $row = sql_fetch("select count(*) as total from $write_table where wr_1='{$member['mb_id']}' and wr_id = $wr_id order by wr_num, wr_reply");
        if ($row['total'] < 1) goto_url(G5_BBS_URL.'/board.php?bo_table='.$bo_table);
    }
}
